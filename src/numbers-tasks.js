...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modolus1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modolus2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalar / (modolus1 * modolus2));
}
...
function getFibonacciNumber(index) {
  ...
  let curSum = 1;
  let nextSum;
  if (index === 0) return prevSum;
  if (index === 1) return curSum;
  for (let i = 2; i <= index; i += 1) {
    nextSum = prevSum + curSum;
    prevSum = curSum;
    curSum = nextSum;
  }
  return nextSum;
